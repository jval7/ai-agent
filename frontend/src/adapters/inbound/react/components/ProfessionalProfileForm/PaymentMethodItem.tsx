  ...
  return (
    <div className="space-y-3">
      <div className="flex gap-3">
        <formFieldModule.FormField htmlFor={`pm-currency-${props.value.methodName}`} label="Moneda">
          <input
            className={`${INPUT_CLASS} w-24`}
            disabled={disabled}
            id={`pm-currency-${props.value.methodName}`}
            maxLength={3}
            onChange={(e) => {
        ...
        <div className="flex-1">
          <formFieldModule.FormField
            htmlFor={`pm-method-name-${props.value.methodName}`}
            label="Medio de pago"
          >
            <input
              className={INPUT_CLASS}
              disabled={disabled}
              id={`pm-method-name-${props.value.methodName}`}
              onChange={(e) => {
      ...
      <formFieldModule.FormField
        htmlFor={`pm-holder-${props.value.methodName}`}
        label="Titular de la cuenta"
      >
        <input
          className={INPUT_CLASS}
          disabled={disabled}
          id={`pm-holder-${props.value.methodName}`}
      ...
      <formFieldModule.FormField
        ...
        htmlFor={`pm-instructions-${props.value.methodName}`}
        label="Instrucciones / datos de pago"
      >
        <textarea
          className={TEXTAREA_CLASS}
          disabled={disabled}
          id={`pm-instructions-${props.value.methodName}`}
      ...
      <formFieldModule.FormField
        ...
        htmlFor={`pm-applies-${props.value.methodName}`}
        label="Aplica cuando"
      >
        <input
          className={INPUT_CLASS}
          disabled={disabled}
          id={`pm-applies-${props.value.methodName}`}
