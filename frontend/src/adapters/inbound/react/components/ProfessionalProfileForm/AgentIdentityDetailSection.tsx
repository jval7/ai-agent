...
export function AgentIdentityDetailSection() {
  ...
  const [identity, setIdentity] =
    reactModule.useState<agentModel.AssistantIdentity>(EMPTY_IDENTITY);
  ...
  reactModule.useEffect(() => {
    if (profileQuery.data !== undefined) {
      setIdentity(profileQuery.data.identity ?? EMPTY_IDENTITY);
      setIsDirty(false);
    }
  }, [profileQuery.data]);
  ...
  const saveMutation = reactQueryModule.useMutation({
    ...
    onSuccess: async () => {
      ...
      setSuccessMessage("Identidad guardada correctamente.");
      setIsDirty(false);
    }
  });
