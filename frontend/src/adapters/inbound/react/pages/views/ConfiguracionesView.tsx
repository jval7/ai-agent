...
import * as useAgentSettingsQueryModule from "@adapters/inbound/react/hooks/useAgentSettingsQuery";
...
function buildSidebarGroups(isAdmin: boolean): settingsSidebarModule.SidebarGroup[] {
  const groups: settingsSidebarModule.SidebarGroup[] = [];
...
function resolveSectionFromParams(params: URLSearchParams, allSectionIds: Set<string>): SectionId {
  const section = params.get("section");
  if (section !== null && allSectionIds.has(section)) return section as SectionId;
...
export function ConfiguracionesView({ tenantId }: { tenantId?: string }) {
  ...
  const sidebarGroups = reactModule.useMemo(() => buildSidebarGroups(isAdmin), [isAdmin]);
  const allSectionIds = reactModule.useMemo(
    () => new Set<string>(sidebarGroups.flatMap((g) => g.items.map((i) => i.id))),
    [sidebarGroups]
  );
  ...
  const [activeSection, setActiveSection] = reactModule.useState<SectionId>(() =>
    resolveSectionFromParams(new URLSearchParams(window.location.search), allSectionIds)
  );
  const [mobileView, setMobileView] = reactModule.useState<"list" | "detail">("list");

  reactModule.useEffect(() => {
    const resolved = resolveSectionFromParams(searchParams, allSectionIds);
    setActiveSection(resolved);
  }, [searchParams, allSectionIds]);
  ...
  // -------------------------------------------------------------------
  // Queries — connections & onboarding (professional only)
  // -------------------------------------------------------------------
  ...
  // -------------------------------------------------------------------
  // Queries — agent settings (admin-aware via hooks)
  // -------------------------------------------------------------------
  const promptQuery = useAgentSettingsQueryModule.useSystemPromptQuery(tenantId);
  const settingsQuery = useAgentSettingsQueryModule.useAgentSettingsQuery(tenantId);
  const professionalProfileQuery =
    useAgentSettingsQueryModule.useProfessionalProfileQuery(tenantId);
  ...
  // -------------------------------------------------------------------
  // Queries — official template status (professional only)
  // -------------------------------------------------------------------
  ...
  function renderDetail(): JSX.Element {
    if (activeSection === "consultorio") {
      return (
        <div className="max-w-3xl space-y-6">
          ...
          {isAdmin ? (
            <div className="rounded-2xl bg-surface-white p-6 shadow-card space-y-3 text-sm text-slate-700">
              <p>
                <strong>Dirección:</strong>{" "}
                {settingsQuery.data?.officeLocation?.address ?? (
            ...
            <div className="space-y-4">
              <div>
                ...
                <textarea
                  ...
                  placeholder="Ej. Avenida Siempre Viva 1234, Edificio Azul, piso 5"
                  rows={3}
                  value={officeAddress}
                />
              </div>
    ...
    if (activeSection === "whatsapp" && !isAdmin) {
      return (
        <div className="max-w-3xl space-y-6">
          <div>
    ...
    if (activeSection === "google-calendar" && !isAdmin) {
      return (
        <div className="max-w-3xl space-y-6">
          <div>
    ...
    if (activeSection === "recordatorios-config" && !isAdmin) {
      return (
        <div className="max-w-3xl space-y-6">
          <div>
            <h2 className="text-xl font-semibold font-display text-brand-ink">
            ...
            {reminderSetupState === "not_configured" ? (
              <div>
                ...
                <p className="mt-2 text-xs text-slate-500">
                  WhatsApp debe aprobar los mensajes antes de empezar a enviarlos.
                </p>
              </div>
            ) : null}
            ...
            {reminderSetupState === "preparing" ? (
              <div className="rounded-xl border border-amber-200 bg-amber-50 p-4">
                ...
                <p className="mt-1 text-xs text-amber-800">
                  WhatsApp está revisando los mensajes. Puede tardar unos minutos.
                </p>
                <button
    ...
    if (activeSection === "plantillas" && !isAdmin) {
      return (
    ...
    if (activeSection === "momento-cobro" && !isAdmin) {
      return (
        <div className="max-w-3xl space-y-6">
          <div>
              ...
              <label className="flex cursor-pointer items-start gap-3">
                ...
                <span className="flex flex-col">
                  ...
                  <span className="mt-0.5 text-xs text-slate-500">
                    El bot exige comprobante de pago para confirmar la cita.
                  </span>
                </span>
              </label>
              <label className="flex cursor-pointer items-start gap-3">
                ...
                <span className="flex flex-col">
                  ...
                  <span className="mt-0.5 text-xs text-slate-500">
                    El bot confirma la cita sin pedir comprobante de pago.
                  </span>
                </span>
              </label>
            </fieldset>
    ...
    if (activeSection === "delay" && !isAdmin) {
      return (
        <div className="max-w-3xl space-y-6">
          <div>
            <h2 className="text-xl font-semibold font-display text-brand-ink">
            ...
            <p className="mt-1 text-sm text-slate-500">
              Tiempo de espera después de procesar un mensaje antes de responder. 0 = sin espera.
            </p>
          </div>
          <div className="rounded-2xl bg-surface-white p-6 shadow-card">
      ...
      return (
        <div className="max-w-5xl space-y-6">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">Solo visible en dev. Read-only.</p>
          </div>
          <div className="rounded-2xl bg-surface-white p-6 shadow-card">
  ...
  return (
    <>
      {!isAdmin && (metaOAuthStatus !== null || googleOAuthStatus !== null) ? (
