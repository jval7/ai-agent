...
import * as useInboxQueryModule from "@adapters/inbound/react/hooks/useInboxQuery";
...
export function InboxView({ tenantId }: InboxViewProps) {
  const isAdminMode = tenantId !== undefined;

  const conversationsQuery = useInboxQueryModule.useConversationsQuery(tenantId);
  const blacklistQuery = useInboxQueryModule.useBlacklistQuery(tenantId);
  const schedulingRequestsQuery = useInboxQueryModule.useInboxSchedulingRequestsQuery(tenantId);
  const agentSettingsQuery = useInboxQueryModule.useInboxAgentSettingsQuery(tenantId);
  const devFeaturesQuery = useInboxQueryModule.useDevFeaturesQuery();
  const patientsQuery = useInboxQueryModule.useInboxPatientsQuery(tenantId);

  const devFeaturesEnabled = !isAdminMode && (devFeaturesQuery.data?.enabled ?? false);
  const sandboxEnabled = !isAdminMode && (devFeaturesQuery.data?.sandbox_enabled ?? false);
  ...
  const controlModeMutation = useInboxQueryModule.useControlModeMutation(tenantId);
  const assistantEnabledMutation = useInboxQueryModule.useAssistantEnabledMutation(tenantId);
  const addBlacklistMutation = useInboxQueryModule.useAddBlacklistMutation(tenantId);
  const removeBlacklistMutation = useInboxQueryModule.useRemoveBlacklistMutation(tenantId);
  const resetMessagesMutation = useInboxQueryModule.useResetMessagesMutation(tenantId);
  const sendMessageMutation = useInboxQueryModule.useSendMessageMutation(tenantId);
  const submitSlotsMutation = useInboxQueryModule.useSubmitSlotsMutation(tenantId);
  const resolvePaymentMutation = useInboxQueryModule.useResolvePaymentMutation(
    selectedConversationId,
    tenantId
  );
  const closeSessionMutation = useInboxQueryModule.useInboxCloseSessionMutation(tenantId);
  const sandboxMutation = useInboxQueryModule.useSandboxMutation();

  const professionalTimezone = "America/Bogota";
  ...
  const messagesQuery = useInboxQueryModule.useConversationMessagesQuery(
    selectedConversationId,
    tenantId
  );
  ...
  const availabilityQuery = useInboxQueryModule.useInboxAvailabilityQuery(
    {
      fromIso: slotPickerMonthFromIso,
      toIso: slotPickerMonthToIso,
      enabled:
        needsOwnerAction && selectedConversationRequest?.status === "AWAITING_CONSULTATION_REVIEW"
    },
    tenantId
  );
