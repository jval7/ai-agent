...
export function ClientsPage() {
  ...
  const patientsQueryKey = ["patients", debouncedSearch] as const;

  const patientsQuery = reactQueryModule.useQuery({
    queryKey: patientsQueryKey,
    queryFn: () =>
      appContainer.patientUseCase.listPatients(
        debouncedSearch.trim() === "" ? undefined : { search: debouncedSearch.trim() }
      )
  });
      ...
      {selectedPatient !== null ? (
        <NewPatientModal
          ...
          onSubmit={async (input: patientModel.UpdatePatientInput) => {
            return appContainer.patientUseCase.updatePatient(selectedPatient.whatsappUserId, input);
          }}
