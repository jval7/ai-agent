...
What lives here:
  ...
  - select_slot_for_confirmation_impl: patient picks one proposed slot;
    ...
    AWAITING_PATIENT_CHOICE with selected_slot_id set (AFTER_SESSION).
...
# ---------------------------------------------------------------------------
# Payment-timing helper (needs agent profile repo)
# ---------------------------------------------------------------------------
...
def select_slot_for_confirmation_impl(
    ...
    payment_timing = get_payment_timing(tenant_id, agent_profile_repository)
    now_value = clock.now()
