"""Session-transition sub-domain for scheduling.

What lives here:
  - handoff_to_human_impl: switches conversation control mode to HUMAN.
  - close_session_impl: closes all active requests and archives the
    conversation subsession (booking or manual close).
  - close_attendance_confirmation: immediately closes an
    AWAITING_ATTENDANCE_CONFIRMATION session when the patient confirms.
  - auto_close_booked_request: triggered by Cloud Task after the auto-close
    delay; archives and closes BOOKED or AWAITING_ATTENDANCE_CONFIRMATION
    requests.

What does NOT live here:
  - Calendar event creation or cancellation (booking.py).
  - Payment approval (payment_approval.py).
  - Slot selection / proposals (slot_proposals.py).
"""
...
def handoff_to_human_impl(
    ...
    """Switch conversation control mode to HUMAN and annotate open requests."""
...
def close_session_impl(
    ...
    """Close all active requests and archive the conversation subsession."""
    request_list = scheduling_repository.list_requests_by_conversation(
        tenant_id,
        conversation_id,
    )
    ...
    booked_request: scheduling_request_entity.SchedulingRequest | None = None
...
def auto_close_booked_request(
    ...
    """Auto-close a BOOKED or AWAITING_ATTENDANCE_CONFIRMATION request after timeout."""
