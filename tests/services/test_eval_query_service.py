...
def test_list_capabilities_returns_12_items() -> None:
    """El glossary tiene exactamente 12 capabilities."""
    svc, _ = _make_service()
...
def test_list_capabilities_categories_distribution() -> None:
    """2 location, 2 cohort, 7 behavior, 1 bot_behavior."""
    svc, _ = _make_service()
    caps = svc.list_capabilities()
    ...
    assert by_category.get("bot_behavior") == 1
